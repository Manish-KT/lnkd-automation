...
COLUMNS = [
    ...
    "Title",
    "Profile URL",
    "Status",          # pending | sent | sent_without_note | skipped | failed | already_connected
    "Note Sent",
    "Keyword",
    "Error",
]
...
STATUS_FILL = {
    ...
    "sent_without_note": PatternFill("solid", fgColor="C6EFCE"), # Added to match LinkedIn script output
...
class Tracker:
    ...
    def _load_processed_companies(self) -> set[str]:
        ...
        for row in self.ws.iter_rows(min_row=2, values_only=True):
            if row and row[company_col - 1]:
                companies.add(str(row[company_col - 1]).strip().lower())
        return companies
    ...
    def sent_today(self) -> int:
        ...
        for row in self.ws.iter_rows(min_row=2, values_only=True):
            ...
            if ts.startswith(today) and status in ("sent", "sent_without_note"):
                count += 1
        return count

    def record(self, contact: Contact) -> None:
        """Appends a record to memory. Call save() explicitly to write to disk."""
        self.ws.append([
            contact.timestamp,
            contact.company,
            contact.company_url,
