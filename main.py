...
from app.tracker import Contact, Tracker
...
def run(cfg: dict, dry_run: bool, headless: bool) -> int:
    ...
    tracker = Tracker(paths["tracker_file"])
        ...
        for keyword in search_cfg["keywords"]:
            ...
            for company in companies:
                ...
                for pop in li.iter_employees(
                    ...
                    note = render_note(note_template, first_name=first_name, company=company.name) if send_note else ""
                    ...
                    if dry_run:
                        ...
                        log.info("   DRY RUN -> %s (%s)", person.name, person.title)
                    else:
                        try:
                            ...
                            log.exception("Connect failed for %s", person.profile_url)

                        log.info("   %s: %s — %s", contact.status, person.name, person.title)

                    tracker.record(contact)
                    processed += 1
                    random_sleep(delays["between_people_min"], delays["between_people_max"])
                
                # Save batch after each company is processed instead of every person
                tracker.save()
                random_sleep(delays["between_companies_min"], delays["between_companies_max"])

        return sent_this_run

    finally:
        tracker.save() # Final save to catch any lingering records before quitting
        li.quit()


def main() -> int:
    parser = argparse.ArgumentParser(description="LinkedIn non-profit outreach automation.")
    parser.add_argument("--config", default="config/config.yaml")
    parser.add_argument("--dry-run", action="store_true", help="Scrape and log only; do not send connection requests.")
    parser.add_argument("--headless", action="store_true", help="Run Chrome headless (not recommended for first run).")
    ...
    sent = run(cfg, dry_run=args.dry_run, headless=args.headless)
